...
protected function pProdutosDANFE($x = 0, $y = 0, $h = 0)
{
    ...
    if ($qtdItens > 0) {
        foreach ($this->venda->itens as $p) {
            ...
            $nitem      = 1;
            $cProd      = $p->produto->id;
            // $xProd      = $p->produto->nome . ($p->observacao != "" ? " [OBS: $p->observacao]" : "" );
            if($p->itemPedido != null) $xProd      = $p->itemPedido->nomeDoProduto();
            else $xProd      = $p->produto->nome;
            $qCom       = $p->quantidade;
...
protected function pTotalDANFE($x = 0, $y = 0, $h = 0)
{
    ...
    $aFont = array('font'=>$this->fontePadrao, 'size'=>7, 'style'=>'B');
    $this->pTextBox($x, $yTotalFinal, $wColEsq, $hLinha, $texto, $aFont, 'T', 'L', 0, '', false);
    $texto = Carbon::parse($this->venda->data_registroo)->format('d/m/Y H:i:s');
    $aFont = array('font'=>$this->fontePadrao, 'size'=>7, 'style'=>'B');
    $this->pTextBox($xValor, $yTotalFinal, $wColDir, $hLinha, $texto, $aFont, 'T', 'R', 0, '', false);
